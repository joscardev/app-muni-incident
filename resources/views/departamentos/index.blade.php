        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                ...
                <div class="flex justify-between items-center px-4 py-3 border-b">
                    ...
                    <a href="{{ route('departamentos.create') }}" class="bg-blue-500 text-white px-4 py-2 rounded">Crear
                        Nuevo</a>
                </div>
                    ...
                    <table class="w-full border-collapse">
                        ...
                        <tbody>
                            @foreach ($departamentos as $departamento)
                                <tr>
                                    <td class="border px-4 py-2 text-white dark:text-gray-200">{{ $departamento->id }}
                                    </td>
                                    <td class="border px-4 py-2 text-white dark:text-gray-200">
                                        {{ $departamento->name }}</td>
                                    <td class="border px-4 py-2 text-white dark:text-gray-200">
                                        {{ $departamento->descripcion }}</td>
                                    <td class="border px-4 py-2 text-white dark:text-gray-200">
                                        {{ $departamento->created_at ? $departamento->created_at->format('d-m-Y H:i:s') : 'No disponible' }}
                                    </td>
                                    <td class="border px-4 py-2 text-white dark:text-gray-200">
                                        {{ $departamento->updated_at ? $departamento->updated_at->format('d-m-Y H:i:s') : 'No disponible' }}
                                    </td>
                                    ...
                                    <td class="border px-4 py-2">
                                        <div class="flex justify-center">
                                            ...
                                            <button type="button"
                                                ...
                                                onclick="confirmDelete('{{ $departamento->id }}')">Delete</button>
                                        </div>
                                    </td>
                                </tr>
                            @endforeach
