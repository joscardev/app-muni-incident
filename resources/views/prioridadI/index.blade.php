        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                ...
                <div class="flex justify-between items-center px-4 py-3 border-b">
                    <h3 class="text-gray-800 dark:text-gray-200">Listado de prioridad</h3>
                    <a href="{{ route('prioridadI.create') }}" class="bg-blue-500 text-white px-4 py-2 rounded">Crear
                        Nuevo</a>
                </div>
                    ...
                    <table class="w-full border-collapse">
                        ...
                        <tbody>
                            @foreach ($prioridad as $prioridad)
                                <tr>
                                    <td class="border px-4 py-2 text-white dark:text-gray-200">{{ $prioridad->id }}
                                    </td>
                                    <td class="border px-4 py-2 text-white dark:text-gray-200">
                                        {{ $prioridad->nombre }}</td>
                                    <td class="border px-4 py-2 text-white dark:text-gray-200">
                                        {{ $prioridad->created_at ? $prioridad->created_at->format('d-m-Y H:i:s') : 'No disponible' }}
                                    </td>
                                    <td class="border px-4 py-2 text-white dark:text-gray-200">
                                        {{ $prioridad->updated_at ? $prioridad->updated_at->format('d-m-Y H:i:s') : 'No disponible' }}
                                    </td>

                                    <!-- Botones editar y eliminar -->
                                    <td class="border px-4 py-2">
                                        <div class="flex justify-center">
                                            <a href="{{ route('prioridadI.edit', $prioridad->id) }}"
                                            ...
                                            <button type="button"
                                                ...
                                                onclick="confirmDelete('{{ $prioridad->id }}')">Delete</button>
                                        </div>
                                    </td>
                                </tr>
                            @endforeach
