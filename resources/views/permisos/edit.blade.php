    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                ...
                <form method="POST" action="{{ route('permisos.update', $permissions->id) }}"
                    class="space-y-6 p-6 bg-white shadow-md rounded-lg dark:bg-gray-800">
                    @csrf
                    @method('PUT')
                    <div>
                        <label for="name" class="block text-sm font-medium text-gray-700 dark:text-gray-200">Permiso
                            Name:</label>
                        <input value="{{ $permissions->name }}" type="text" name="name" id="name" required 
