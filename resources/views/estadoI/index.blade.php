        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                ...
                <div class="flex justify-between items-center px-4 py-3 border-b">
                    ...
                    <a href="{{ route('estadoI.create') }}" class="bg-blue-500 text-white px-4 py-2 rounded">Crear
                        Nuevo</a>
                </div>
                    ...
                    <table class="w-full border-collapse">
                        ...
                        <tbody>
                            @foreach ($estados as $estado)
                                <tr>
                                    <td class="border px-4 py-2 text-white dark:text-gray-200">{{ $estado->id }}
                                    </td>
                                    <td class="border px-4 py-2 text-white dark:text-gray-200">
                                        {{ $estado->nombre }}</td>
                                    <td class="border px-4 py-2 text-white dark:text-gray-200">
                                        {{ $estado->created_at ? $estado->created_at->format('d-m-Y H:i:s') : 'No disponible' }}
                                    </td>
                                    <td class="border px-4 py-2 text-white dark:text-gray-200">
                                        {{ $estado->updated_at ? $estado->updated_at->format('d-m-Y H:i:s') : 'No disponible' }}
                                    </td>

                                    <!-- Botones editar y eliminar -->
                                    <td class="border px-4 py-2">
                                        <div class="flex justify-center">
                                            <a href="{{ route('estadoI.edit', $estado->id) }}"
                                            ...
                                            <button type="button"
                                                ...
                                                onclick="confirmDelete('{{ $estado->id }}')">Delete</button>
                                        </div>
                                    </td>
                                </tr>
                            @endforeach
